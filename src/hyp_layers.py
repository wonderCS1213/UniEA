...
from src.att_layers import DenseAtt
...
def get_dim_act_curv(args):
    ...
    acts = [act] * (args.layer - 1)
    print(acts)
    dims = [args.feat_dim] + ([args.dim] * (args.layer - 1))
    print(dims)
    args.task = "lp"
    if args.task in ['lp', 'rec']:
        dims += [args.dim]
        acts += [act]
        n_curvatures = args.layer
    else:
        n_curvatures = args.layer - 1
    if args.c is None:
...
class HypLinear(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(drop_weight.shape, x.shape, self.c.shape, "sa-----------")
