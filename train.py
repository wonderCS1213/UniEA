...
os.environ['CUDA_VISIBLE_DEVICES'] = "1"  # （代表仅使用第2，3号GPU）
...
parser.add_argument("--model", type=str, default='UniEA', help="the model's name", choices=['gaea', 'gat', 'gcn', 'UniEA'])
...
parser.add_argument("--fold", type=int, default=4, help="the fold cross-validation")
...
parser.add_argument("--eval_metric", type=str, default="csls", help="the distance metric of entity pairs", choices=["cityblock", "euclidean", "csls", "inner"])
...
parser.add_argument("--neg_metric", type=str, default="csls", choices=["cityblock", "euclidean", "csls", "inner"]) # same as BootEA
...
parser.add_argument("--csls_k", type=int, default=1, help="top k for csls")
parser.add_argument("--train_batch_size", type=int, default=-1, help="train batch_size (-1 means all in)")
parser.add_argument('--patience', type=int, default=500, help='patience default 5')
...
parser.add_argument('--rel_dim', type=int, default=32, help="hidden dimension of relation embeddings")
...
parser.add_argument('--pr', type=float, default=0, help='the edge drop rate')
parser.add_argument('--aug_balance', type=float, default=55, help='the hyper-parameter of consistency loss')
parser.add_argument('--aug_iter', type=int, default=10)
# 双曲
parser.add_argument('--manifold', default='PoincareBall', choices=["Euclidean", "Hyperboloid", "PoincareBall"], type=str)
parser.add_argument('--c', type=float, default=1.0) # None
# parser.add_argument('--r', type=float, default=2.)
# parser.add_argument('--t', type=float, default=1.)
parser.add_argument('--bias', type=int, default=1, help='0 or 1')
parser.add_argument('--alpha', type=float, default=0.2)
parser.add_argument('--use_att', type=int, default=0, help="whether to use hyperbolic attention or not")
parser.add_argument('--local_agg', type=int, default=0, help="whether to local tangent space aggregation or not")
parser.add_argument('--act', type=str, default='relu', help='which activation function to use (or None for no activation)')
parser.add_argument('--n_heads', type=int, default=2, help="hyper number of multi-head attention")
parser.add_argument('--dim', type=int, default=256, help="hyper hidden dimension of relation embeddings")
parser.add_argument('--feat_dim', type=int, default=256, help="hidden dimension of relation embeddings")
...
elif args.model == "UniEA":
    model = UniEA(num_sr=kgdata.kg1_ent_num, num_tg=kgdata.kg2_ent_num, adj_sr=kgdata.tensor_adj1, adj_tg=kgdata.tensor_adj2, rel_num=kgdata.rel_num, rel_adj_sr=kgdata.tensor_rel_adj1, rel_adj_tg=kgdata.tensor_rel_adj2, args=args)
...
def evaluate(test_pair, k, eval_metric, phase="test", model_path=None):
    ...
    model.eval()
    print(args.c, "---------")
    sr_embedding, tg_embedding, hyper_sr_embedding, hyper_tg_embedding = model(phase="eval")
...
try:
    ...
    time_points = [ ]  # 记录时间差
    h_at_1_values = [ ]  # 记录H@1值
    start_time = time.time()  # 记录初始时间点
    for e in range(args.epoch):
        ...
        for _, data in enumerate(train_loader):
            ...
            if args.model == "gaea":
                ...
                loss = loss_fn(sr_embedding, tg_embedding, a1_align, a2_align, neg1_left, neg1_right, neg2_left, neg2_right, neg_samples_size=args.neg_samples_size, loss_norm=args.loss_norm, pos_margin=args.pos_margin, neg_margin=args.neg_margin, neg_param=args.neg_param)
            elif args.model in ["UniEA"]:
                sr_embedding, tg_embedding, hyper_sr_embedding, hyper_tg_embedding = model(phase="norm")
                loss1 = loss_fn(sr_embedding,  tg_embedding, a1_align, a2_align, neg1_left, neg1_right, neg2_left,
                ...
                aug_loss1 = model.contrastive_loss(sr_embedding, hyper_sr_embedding, kgdata.kg1_ent_num)
                aug_loss2 = model.contrastive_loss(tg_embedding, hyper_tg_embedding, kgdata.kg2_ent_num)
                inner_loss1 = model.contrastive_loss(sr_embedding, sr_embedding, kgdata.kg1_ent_num)
                #inner_loss2 = model.contrastive_loss(tg_embedding, tg_embedding, kgdata.kg2_ent_num)
                #loss = loss1 + args.aug_balance * (aug_loss1 + aug_loss2 + inner_loss1 + inner_loss2)
                loss = loss1 + args.aug_balance * (aug_loss1 + aug_loss2 + inner_loss1)
                #loss = loss1 + args.aug_balance * (aug_loss1 + aug_loss2)
                #loss = loss1
            '''multi-loss learning'''
        ...
        if args.val and (e+1) % args.val_iter == 0 and e > 0 and (e+1) >= args.val_start:
            hit_1_score, hit_k_score, mrr, mean = evaluate(test_pair=kgdata.mapped_val_pair, k=args.k, eval_metric=args.eval_metric, phase="eval")
            if best_val < mrr:
